...
function extractNameFromTemplate(value) {
  ...
  const name = value.slice(commaIndex + 2, exclamationIndex).trim();
...
function encodeToRot13(str) {
  const rot13Transform = (char) => {
    ...
    const transformedIndex = (index + 13) % 26;
    return alphabet[transformedIndex];
  };

  // Apply ROT13 transformation to each character in the input string
  const result = str
    .split('')
    .map((char) => rot13Transform(char))
    .join('');
